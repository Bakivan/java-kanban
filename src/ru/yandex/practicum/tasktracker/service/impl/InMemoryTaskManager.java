...
public class InMemoryTaskManager implements TaskManager {
    ...
    private void changeEpicStatus(Epic epic) {
        if (epic.getSubtasksForThisEpic().isEmpty()) {
            ...
            ArrayList<TaskStatus> statusesFromEpicSubtasks = new ArrayList<>();
            ...
            if (statusesFromEpicSubtasks.stream()
                    .anyMatch(status -> status == TaskStatus.IN_PROGRESS)) {
                epic.setStatus(TaskStatus.IN_PROGRESS);
                return;
            } else if (statusesFromEpicSubtasks.stream()
                    .anyMatch(status -> status == TaskStatus.NEW) &&
                    statusesFromEpicSubtasks.stream()
                            .anyMatch(status -> status == TaskStatus.DONE)) {
                epic.setStatus(TaskStatus.IN_PROGRESS);
                return;
            } else if (statusesFromEpicSubtasks.stream()
                    .anyMatch(status -> status == TaskStatus.NEW)) {
